...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched MonyaCode instance.
    ...
    /// When set to false, MonyaCode will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where MonyaCode will always
    ...
    /// What draws window decorations/titlebar, the client application (MonyaCode) or display server
    /// Default: client
...
pub enum WindowDecorations {
    /// MonyaCode draws its own window decorations/titlebar (client-side decoration)
    #[default]
    Client,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting MonyaCode.
    #[default]
    LastSession,
