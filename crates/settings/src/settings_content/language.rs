...
pub struct LanguageSettingsContent {
    ...
    /// MonyaCode's Prettier integration settings.
    ...
    /// when you type '(', MonyaCode will automatically add a closing ')' at the correct position.
    ///
    /// Default: false
    pub use_autoclose: Option<bool>,
    /// Whether to automatically surround text with characters for you. For example,
    /// when you select text and type '(', MonyaCode will automatically surround text with ().
    ///
    /// Default: true
...
#[serde(rename_all = "snake_case")]
pub enum Formatter {
    /// Format files using MonyaCode's Prettier integration (if applicable),
    /// or falling back to formatting via language server.
    #[default]
    Auto,
    /// Format code using MonyaCode's Prettier integration.
    Prettier,
...
pub struct LanguageTaskSettingsContent {
    ...
    pub enabled: Option<bool>,
    /// Use LSP tasks over MonyaCode language extension ones.
    /// If no LSP tasks are returned due to error/timeout or regular execution,
    /// MonyaCode language extension tasks will be used instead.
    ///
    /// Other MonyaCode tasks will still be shown:
    /// * MonyaCode task from either of the task config file
    /// * MonyaCode task from history (e.g. one-off task was spawned before)
