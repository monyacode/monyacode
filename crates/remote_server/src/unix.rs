...
pub static VERSION: LazyLock<&str> = LazyLock::new(|| match *RELEASE_CHANNEL {
    ...
    ReleaseChannel::Dev => option_env!("MONYACODE_COMMIT_SHA").unwrap_or("missing-monyacode-commit-sha"),
});
        ...
        |_, _: TypedEnvelope<proto::GetCrashFiles>, _cx| async move {
            ...
            while let Some(child) = children.next().await {
                ...
                if extension == Some(OsStr::new("panic")) {
                    ...
                    if !filename.starts_with("monyacode") {
                        continue;
                    }
...
pub fn execute_run(
    ...
    app.background_executor()
        .spawn(crashes::init(crashes::InitCrashHandler {
            session_id: id,
            monyacode_version: VERSION.to_owned(),
            binary: "monyacode-remote-server".to_string(),
            release_channel: release_channel::RELEASE_CHANNEL_NAME.clone(),
            commit_sha: option_env!("MONYACODE_COMMIT_SHA").unwrap_or("no_sha").to_owned(),
        }))
        .detach();
...
fn cleanup_old_binaries() -> Result<()> {
    ...
    let prefix = format!("monyacode-remote-server-{}-", release_channel);
