...
use task::{DebugRequest, DebugScenario, LaunchRequest, MonyaCodeDebugConfig, TaskContext, VariableName};
use text::Point;
use util::path;
...
async fn test_debug_session_substitutes_variables_and_relativizes_paths(
    ...
    let test_cases: Vec<(&'static str, &'static str)> = vec![
        ...
        // Path with $MONYACODE_WORKTREE_ROOT - should be substituted without double appending
        (
            format!("$MONYACODE_WORKTREE_ROOT{0}src{0}program", std::path::MAIN_SEPARATOR).leak(),
            path!("/test/worktree/path/src/program"),
        ),
    ];
            ...
            move |client| {
                client.on_request::<dap::requests::Launch, _>({
                    ...
                    move |_, args| {
                        ...
                        let expected_other_field = if input_path.contains("$MONYACODE_WORKTREE_ROOT") {
                            input_path.replace("$MONYACODE_WORKTREE_ROOT", path!("/test/worktree/path"))
                        } else {
...
async fn test_save_debug_scenario_to_file(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let debug_json_content = fs
        .load(path!("/project/.monyacode/debug.jsonc").as_ref())
        .await
...
async fn test_debug_modal_subtitles_with_multiple_worktrees(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    assert_eq!(
        subtitles.as_slice(),
        [path!(".monyacode/debug.jsonc"), path!(".monyacode/debug.jsonc")]
    );
}
...
async fn test_dap_adapter_config_conversion_and_validation(cx: &mut TestAppContext) {
    ...
    let monyacode_config = MonyaCodeDebugConfig {
    ...
    for adapter_name in adapter_names {
        ...
        let mut adapter_specific_config = monyacode_config.clone();
        adapter_specific_config.adapter = adapter_name.to_string().into();

        let debug_scenario = adapter
            .config_from_monyacode_format(adapter_specific_config)
            .await
            .unwrap_or_else(|_| {
                panic!(
                    "Adapter {} should successfully convert from MonyaCode format",
                    adapter_name
                )
            });
