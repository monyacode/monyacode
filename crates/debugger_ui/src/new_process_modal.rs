...
use task::{DebugScenario, MonyaCodeDebugConfig, RevealTarget, VariableName};
use ui::{
...
impl NewProcessModal {
    ...
    fn debug_scenario(&self, debugger: &str, cx: &App) -> Task<Option<DebugScenario>> {
        ...
        let session_scenario = MonyaCodeDebugConfig {
        ...
        cx.spawn(async move |_| adapter?.config_from_monyacode_format(session_scenario).await.ok())
    }
...
impl ConfigureMode {
    pub(super) fn new(window: &mut Window, cx: &mut App) -> Entity<Self> {
        let program = cx.new(|cx| {
            InputField::new(window, cx, "ENV=MonyaCode ~/bin/program --option")
                .label("Program")
                .tab_stop(true)
                .tab_index(1)
        });
        ...
        let cwd = cx.new(|cx| {
            InputField::new(window, cx, "Ex: $MonyaCode_WORKTREE_ROOT")
...
pub(super) struct AttachMode {
    pub(super) definition: MonyaCodeDebugConfig,
...
impl AttachMode {
    pub(super) fn new(
        ...
        let definition = MonyaCodeDebugConfig {
            ...
            this.update(cx, |this, cx| {
                ...
                let hide_vscode = scenarios.iter().any(|(kind, _)| match kind {
                    TaskSourceKind::Worktree {
                        ...
                        id_base: _,
                    } => dir.ends_with(RelPath::unix(".monyacode").unwrap()),
                    _ => false,
                });
...
pub(crate) fn resolve_path(path: &mut String) {
    if path.starts_with('~') {
        ...
        *path = format!("$MONYACODE_WORKTREE_ROOT{}{}", std::path::MAIN_SEPARATOR, strip_path);
    };
}
