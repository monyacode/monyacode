...
use task::{
    BuildTaskDefinition, DebugScenario, MonyaCodeDebugConfig, SaveStrategy, Shell, ShellBuilder, SpawnInTerminal,
    ...
    pub(crate) fn substitute_variables_in_config(config: &mut serde_json::Value, context: &TaskContext) {
        match config {
            ...
            serde_json::Value::String(s) => {
                // Some built-in monyacode tasks wrap their arguments in quotes as they might contain spaces.
                if s.starts_with("\"$MONYACODE_") && s.ends_with('"') {
    ...
    pub(crate) fn resolve_scenario(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if config_is_valid {
                ...
                let zed_config = MonyaCodeDebugConfig {
                    label: label.clone(),
                ...
                let scenario = dap_registry
                    .adapter(&adapter)
                    .with_context(|| anyhow!("{}: is not a valid adapter name", adapter))?.config_from_monyacode_format(zed_config)
                    .await?;
                ...
                anyhow::bail!("MonyaCode cannot determine how to run this debug scenario. `build` field was not provided and Debug Adapter won't accept provided configuration because: {e}");
            };
