...
pub fn init(cx: &mut App) {
    ...
    if let Some(extension_events) = extension::ExtensionEvents::try_global(cx) {
        cx.subscribe(&extension_events, |_, evt, cx| {
            ...
            cx.update_global::<SchemaStore, _>(|schema_store, cx| {
                schema_store.notify_schema_changed("monyacode://schemas/settings", cx);
            });
        })
        .detach();
    }
    ...
    cx.observe_global::<dap::DapRegistry>(|cx| {
        cx.update_global::<SchemaStore, _>(|schema_store, cx| {
            schema_store.notify_schema_changed("monyacode://schemas/debug_tasks", cx);
        });
    })
    .detach();
}
...
pub async fn resolve_schema_request(
    ...
    let path = uri.strip_prefix("monyacode://schemas/").context("Invalid URI")?;
                ...
                .and_then(|r| {
                    r.strip_prefix(
                        LSP_SETTINGS_SCHEMA_URL_PREFIX
                            .strip_prefix("monyacode://schemas/settings/")
                            .unwrap(),
                    )
                })
...
pub fn all_schema_file_associations(
    ...
    let mut file_associations = serde_json::json!([
        {
            ...
            "url": "monyacode://schemas/settings",
        },
        {
            "fileMatch": [schema_file_match(paths::keymap_file())],
            "url": "monyacode://schemas/keymap",
        },
        {
            "fileMatch": [
        ...
        },
        {
            ...
            "url": "monyacode://schemas/debug_tasks",
        },
        {
            "fileMatch": [
                schema_file_match(
            ...
            "url": "monyacode://schemas/snippets",
        },
        {
            "fileMatch": ["tsconfig.json"],
            "url": "monyacode://schemas/tsconfig"
        },
        {
            "fileMatch": ["package.json"],
            "url": "monyacode://schemas/package_json"
        },
        {
            "fileMatch": &jsonc_globs,
            "url": "monyacode://schemas/jsonc"
        },
    ]);
    ...
    #[cfg(debug_assertions)]
    {
        file_associations.as_array_mut().unwrap().push(serde_json::json!({
            "fileMatch": [
                "monyacode-inspector-style.json"
            ],
            "url": "monyacode://schemas/monyacode_inspector_style"
        }));
    }
    ...
    file_associations.as_array_mut().unwrap().extend(
        ...
        cx.all_action_names().into_iter().map(|&name| {
            ...
            serde_json::json!({
                "fileMatch": [file_name],
                "url": format!("monyacode://schemas/action/{normalized_name}")
            })
        }),
    );
