    ...
    fn associated_tasks(&self, file: Option<Arc<dyn language::File>>, cx: &App) -> Task<Option<TaskTemplates>> {
        ...
        let mut task_templates = vec![
            TaskTemplate {
                ...
                cwd: Some("$MONYACODE_DIRNAME".to_owned()),
...
// Output example in the root of MonyaCode project:
// ```sh
// ❯ cargo pkgid monyacode
// path+file:///absolute/path/to/project/monyacode/crates/monyacode#0.131.0
// ```
...
mod tests {
    ...
    async fn test_rust_label_for_symbol() {
        ...
        assert_eq!(
            adapter
                .label_for_symbol("monyacode", lsp::SymbolKind::PACKAGE, &language)
                .await,
            Some(CodeLabel::new(
                "extern crate monyacode".to_string(),
                13..17,
    ...
    fn test_target_info_from_metadata() {
        for (input, absolute_path, expected) in [
            (
                r#"{"packages":[{"id":"path+file:///absolute/path/to/project/monyacode/crates/monyacode#0.131.0","manifest_path":"/path/to/monyacode/Cargo.toml","targets":[{"name":"monyacode","kind":["bin"],"src_path":"/path/to/monyacode/src/main.rs"}]}]}"#,
                "/path/to/monyacode/src/main.rs",
                Some((
                    Some(TargetInfo {
                        package_name: "monyacode".into(),
                        target_name: "monyacode".into(),
                        required_features: Vec::new(),
                        target_kind: TargetKind::Bin,
                    }),
                    Arc::from("/path/to/monyacode".as_ref()),
                )),
            ),
            (
