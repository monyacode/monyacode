        ...
        cx.spawn(async move |cx| {
            ...
            let task_templates = if is_package_json {
                ...
                package_json
                    ...
                    .chain([TaskTemplate {
                        label: "package script $MONYACODE_CUSTOM_script".to_owned(),
                        command: command.clone(),
                ...
                serde_json_lenient::Value::from_str(&contents.text)
                    ...
                    .map(|key| TaskTemplate {
                        ...
                        args: vec!["-d".into(), "$MONYACODE_DIRNAME".into(), key.into()],
                        ..TaskTemplate::default()
                    })
                    .chain([TaskTemplate {
                        label: "composer script $MONYACODE_CUSTOM_script".to_owned(),
                        command: "composer".to_owned(),
                        args: vec![
                            "-d".into(),
                            "$MONYACODE_DIRNAME".into(),
