...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary MonyaCode should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, MonyaCode will download its own copy of Node.
    pub ignore_system_version: bool,
    /// If disabled, MonyaCode will not download its own copy of Node.
    pub allow_binary_download: bool,
    /// If disabled, MonyaCode will not download its own copy of Prettier.
    pub allow_prettier_download: bool,
    /// If disabled, MonyaCode will not try to install language servers using NPM.
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in MonyaCode.
...
/// SettingsObserver observers changes to .monyacode/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/monyacode/{settings, task}.json and sends the content
/// upstream.
                        ...
                        Some(
                            async move {
                                    ...
                                    || abs_path.ends_with(local_vscodium_tasks_file_relative_path().as_std_path())
                                {
                                    ...
                                    let monyacode_tasks = TaskTemplates::try_from(vscode_tasks).with_context(|| {
                                        format!("converting VSCode/VSCodium tasks into MonyaCode ones, file {abs_path:?}")
                                    })?;
                                    serde_json::to_string(&monyacode_tasks)
                                        .with_context(|| format!("serializing MonyaCode tasks into JSON, file {abs_path:?}"))
                                    ...
                                    || abs_path.ends_with(local_vscodium_launch_file_relative_path().as_std_path())
                                {
                                    ...
                                    let monyacode_tasks = DebugTaskFile::try_from(vscode_tasks).with_context(|| {
                                        format!(
                                            "converting VSCode/VSCodium debug tasks into MonyaCode ones, file {abs_path:?}"
                                        )
                                    })?;
                                    serde_json::to_string(&monyacode_tasks)
                                        .with_context(|| format!("serializing MonyaCode tasks into JSON, file {abs_path:?}"))
