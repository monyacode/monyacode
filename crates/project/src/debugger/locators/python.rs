...
impl DapLocator for PythonLocator {
    ...
    async fn create_scenario(
        ...
        let valid_program = build_config.command.starts_with("$MONYACODE_")
...
mod test {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$MONYACODE_FILE'".into(),
            command: "$MONYACODE_CUSTOM_PYTHON_ACTIVE_MONYACODE_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$MONYACODE_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: Default::default(),
            cwd: Some("$MONYACODE_WORKTREE_ROOT".into()),
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$MONYACODE_CUSTOM_PYTHON_ACTIVE_MONYACODE_TOOLCHAIN",
                "args": [],
                "cwd": "$MONYACODE_WORKTREE_ROOT",
                "module": "$MONYACODE_CUSTOM_PYTHON_MODULE_NAME",
            }),
            tcp_connection: None,
        };
