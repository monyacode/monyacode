//! Provides constructs for the MonyaCode app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("MONYACODE_RELEASE_CHANNEL")
            .unwrap_or_else(|_| include_str!("../../monyacode/RELEASE_CHANNEL").trim().to_string())
    } else {
        include_str!("../../monyacode/RELEASE_CHANNEL").trim().to_string()
    }
});
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "MonyaCode-Editor-Dev",
        ReleaseChannel::Stable => "MonyaCode-Editor-Stable",
    }
}

/// The Git commit SHA that MonyaCode was built at.
...
/// The version of MonyaCode.
pub struct AppVersion;

impl AppVersion {
    /// Load the app version from env.
    pub fn load(pkg_version: &str) -> SemanticVersion {
        if let Ok(from_env) = env::var("MONYACODE_APP_VERSION") {
            from_env.parse().expect("invalid MONYACODE_APP_VERSION")
        } else {
...
/// A MonyaCode release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of MonyaCode.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "MonyaCode Dev",
            ReleaseChannel::Stable => "MonyaCode",
        }
    }
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for MonyaCode on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "app.liten.MonyaCode-Dev",
            ReleaseChannel::Stable => "app.liten.MonyaCode",
        }
    }
