...
impl IExplorerCommand_Impl for ExplorerCommandInjector_Impl {
    fn GetTitle(&self, _: Ref<IShellItemArray>) -> Result<windows_core::PWSTR> {
        let command_description = retrieve_command_description().unwrap_or(HSTRING::from("Open with MonyaCode"));
        unsafe { SHStrDupW(&command_description) }
    }

    fn GetIcon(&self, _: Ref<IShellItemArray>) -> Result<windows_core::PWSTR> {
        let Some(monyacode_exe) = get_monyacode_exe_path() else {
            return Err(E_FAIL.into());
        };
        unsafe { SHStrDupW(&HSTRING::from(monyacode_exe)) }
    }
    ...
    fn Invoke(&self, psiitemarray: Ref<IShellItemArray>, _: Ref<IBindCtx>) -> Result<()> {
        ...
        for idx in 0..count {
            ...
            std::process::Command::new(&monyacode_exe)
                .arg(&item_path)
                .spawn()
...
fn get_monyacode_install_folder() -> Option<PathBuf> {
...
#[inline]
fn get_monyacode_exe_path() -> Option<String> {
    get_monyacode_install_folder().map(|path| path.join("MonyaCode.exe").to_string_lossy().into_owned())
}

#[inline]
fn retrieve_command_description() -> Result<HSTRING> {
    const REG_PATH: &str = "Software\\Classes\\MonyaCodeContextMenu";
