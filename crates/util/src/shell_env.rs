...
async fn capture_unix(
    ...
    let binary_path = super::get_shell_safe_binary_path(shell_kind)?;
    ...
    command_string.push_str(&format!("{} --printenv {}", binary_path, redir));
    ...
    // Parse the JSON output from monyacode --printenv
