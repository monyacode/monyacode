...
/// `MONYACODE_ALLOW_ROOT` environment variable.
#[cfg(unix)]
pub fn prevent_root_execution() {
    let is_root = nix::unistd::geteuid().is_root();
    let allow_root = std::env::var("MONYACODE_ALLOW_ROOT").is_ok_and(|val| val == "true");

    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running MonyaCode as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of MonyaCode.
       It is untested and not recommended, don't complain when things break.
       If you wish to proceed anyways, set `MONYACODE_ALLOW_ROOT=true` in your environment."
        );
        std::process::exit(1);
    }
}
...
pub fn get_shell_safe_binary_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
    ...
    binary_path
        ...
        .context("Failed to shell-escape MonyaCode executable path.")
}

/// Returns a path for the monyacode cli executable, this function
/// should be called from the executable, not monyacode-cli.
pub fn get_monyacode_cli_path() -> Result<PathBuf> {
    ...
    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        // On macOS, the executable and monyacode-cli are inside the app bundle,
        // so here ./cli is for both installed and development builds.
        &["./cli"]
    } else if cfg!(target_os = "windows") {
        // bin/monyacode.exe is for installed builds, ./cli.exe is for development builds.
        &["bin/monyacode.exe", "./cli.exe"]
        ...
        anyhow::bail!("unsupported platform for determining monyacode-cli path");
    };
    ...
    possible_locations
        ...
        .with_context(|| {
            format!(
                "could not find monyacode-cli from any of: {}",
...
pub fn measure<R>(label: &str, f: impl FnOnce() -> R) -> R {
    static MONYACODE_MEASUREMENTS: OnceLock<bool> = OnceLock::new();
    let monyacode = MONYACODE_MEASUREMENTS.get_or_init(|| {
        env::var("MONYACODE_MEASUREMENTS")
