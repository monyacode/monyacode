...
const LIGHT_THEMES: [&str; 3] = ["Melange Light", "Rosé Pine Dawn", "Ayu Light"];
const DARK_THEMES: [&str; 3] = ["Melange Dark", "Rosé Pine", "Ayu Dark"];
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    let base_keymap = match BaseKeymap::get_global(cx) {
        BaseKeymap::Emacs => Some(0),
        BaseKeymap::JetBrains => Some(1),
        BaseKeymap::VSCode => Some(2),
    ...
    return v_flex().gap_2().child(Label::new("Base Keymap")).child(
        ToggleButtonGroup::three_rows(
            ...
            [
                ...
                ToggleButtonWithIcon::new("TextMate", IconName::Flower, |_, _, cx| {
