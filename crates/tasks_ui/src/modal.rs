    ...
    pub fn tasks_loaded(
        ...
        let hide_vscode = current_resolved_tasks.iter().any(|(kind, _)| match kind {
            TaskSourceKind::Worktree {
                ...
                id_base: _,
            } => dir.file_name().is_some_and(|name| name == ".monyacode"),
            _ => false,
        });
        ...
        fs.insert_tree(
            ...
            json!({
                ".monyacode": {
                    "tasks.jsonc": r#"[
                        {
                            "label": "hello from $MONYACODE_FILE:$MONYACODE_ROW:$MONYACODE_COLUMN",
                            "command": "echo",
                            "args": ["hello", "from", "$MONYACODE_FILE", ":", "$MONYACODE_ROW", ":", "$MONYACODE_COLUMN"]
                        },
                        {
                            "label": "opened now: $MONYACODE_WORKTREE_ROOT",
                            "command": "echo",
                            "args": ["opened", "now:", "$MONYACODE_WORKTREE_ROOT"]
                        }
                    ]"#,
                },
            ...
            language_registry.add(Arc::new(
                ...
                .with_context_provider(Some(Arc::new(ContextProviderWithTasks::new(TaskTemplates(
                    vec![
                        ...
                        TaskTemplate {
                            label: "TypeScript task from file $MONYACODE_FILE".to_string(),
                            command: "npm run build".to_string(),
                            ..TaskTemplate::default()
                        },
                        TaskTemplate {
                            label: "Another task from file $MONYACODE_FILE".to_string(),
