...
pub async fn init(crash_init: InitCrashHandler) {
    let generate_minidumps = match env::var("MONYACODE_GENERATE_MINIDUMPS") {
        Ok(v) => v == "true" || v == "1",
        Err(_) => false,
    };
    ...
    // server or interactive monyacode instance. This solves an issue where occasionally the socket
    ...
    let socket_name = paths::temp_dir().join(format!("monyacode-crash-handler-{app_pid}"));
    #[cfg(not(target_os = "windows"))]
    ...
    fn on_message(&self, kind: u32, buffer: Vec<u8>) {
        match kind {
            ...
            3 => {
                ...
                // on each new window. in theory all monyacode windows should be using the same
                // GPU so this is fine.
