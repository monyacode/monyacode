...
impl VsCodeTaskDefinition {
    fn into_monyacode_format(self, replacer: &EnvVariableReplacer) -> anyhow::Result<Option<TaskTemplate>> {
...
impl TryFrom<VsCodeTaskFile> for TaskTemplates {
    ...
    fn try_from(value: VsCodeTaskFile) -> Result<Self, Self::Error> {
        ...
        let templates = value
            .tasks
            .into_iter()
            .filter_map(|vscode_definition| vscode_definition.into_monyacode_format(&replacer).log_err().flatten())
            .collect();
        Ok(Self(templates))
    }
}
...
mod tests {
    ...
    fn test_variable_substitution() {
        ...
        let replacer = EnvVariableReplacer::new(HashMap::from_iter([("PATH".to_owned(), "MONYACODE_PATH".to_owned())]));
        assert_eq!(replacer.replace("Food"), "Food");
        assert_eq!(
            replacer.replace("$PATH is an environment variable"),
            "${MONYACODE_PATH} is an environment variable"
        );
        assert_eq!(replacer.replace("${PATH}"), "${MONYACODE_PATH}");
        assert_eq!(replacer.replace("${PATH:food}"), "${MONYACODE_PATH:food}");
    }
    ...
    fn can_deserialize_ts_tasks() {
        ...
        let expected = vec![
            ...
            TaskTemplate {
                ...
                command: "node".to_string(),
                args: vec![
                    "${MONYACODE_WORKTREE_ROOT}/node_modules/typescript/lib/tsc.js".to_string(),
                    "--build".to_string(),
                    "${MONYACODE_WORKTREE_ROOT}/src".to_string(),
                    "--watch".to_string(),
                ],
                ..Default::default()
            },
