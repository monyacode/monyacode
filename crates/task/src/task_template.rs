...
use crate::{
    AttachRequest, MONYACODE_VARIABLE_NAME_PREFIX, ResolvedTask, RevealTarget, Shell, SpawnInTerminal, TaskContext,
    TaskId, VariableName, serde_helpers::non_empty_string_vec,
};

/// A template definition of a MonyaCode task to run.
...
/// contains all relevant MonyaCode state in task variables.
...
impl TaskTemplate {
    ...
    /// If any replacement fails or the new string substitutions still have [`MONYACODE_VARIABLE_NAME_PREFIX`],
    /// `None` is returned.
    ///
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        } else if variable_name.starts_with(MONYACODE_VARIABLE_NAME_PREFIX) {
            // Unknown MONYACODE variable - use default if available
            if !default.is_empty() {
...
mod tests {
    ...
    fn test_template_variables_resolution() {
        ...
        for i in 0..all_variables.len() {
            ...
            assert_eq!(
                ...
                "If any of the MonyaCode task variables is not substituted, the task should not be resolved, but got some resolution without the variable {removed_variable:?} (index {i})"
            );
        }
    }
    ...
    #[test]
    fn test_errors_on_missing_monyacode_variable() {
        let task = TaskTemplate {
            label: "My task".into(),
            command: "echo".into(),
            args: vec!["$MONYACODE_VARIABLE".into()],
            ..TaskTemplate::default()
        };
    ...
    fn test_variable_default_values() {
        let task_with_defaults = TaskTemplate {
            ...
            args: vec![
                "${MONYACODE_MISSING_VAR:default_value}".to_string(),
                format!("${{{}}}", VariableName::Row.to_string() + ":42"),
            ],
            ..TaskTemplate::default()
        };

        // Test 1: When MONYACODE_FILE exists, should use actual value and ignore default
        ...
        assert_eq!(
            ...
            "echo actual_file.rs",
            "Should use actual MONYACODE_FILE value, not default"
        );
        assert_eq!(
        ...
        // Test 2: When MONYACODE_FILE doesn't exist, should use default value
        ...
        assert_eq!(
            resolved.resolved.command.unwrap(),
            "echo fallback.txt",
            "Should use default value when MONYACODE_FILE is missing"
        );
        assert_eq!(
            resolved.resolved.args,
        ...
        // Test 3: Missing MONYACODE variable without default should fail
        let task_no_default = TaskTemplate {
            label: "test no default".to_string(),
            command: "${MONYACODE_MISSING_NO_DEFAULT}".to_string(),
            ..TaskTemplate::default()
        };
        ...
        assert!(
            task_no_default
                ...
                .is_none(),
            "Should fail when MONYACODE variable has no default and doesn't exist"
        );
    }
}
