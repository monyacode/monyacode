//! Baseline interface of Tasks in MonyaCode: all tasks in MonyaCode are intended to use those for implementing their own logic.
...
pub use debug_format::{
    ...
    MonyaCodeDebugConfig, Request, TcpArgumentsTemplate,
};
...
/// Contains all information needed by MonyaCode to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a MonyaCode's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(MONYACODE_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            "COLUMN" => Self::Column,
            _ => {
                if let Some(custom_name) = without_prefix.strip_prefix(MONYACODE_CUSTOM_VARIABLE_NAME_PREFIX) {
...
pub const MONYACODE_VARIABLE_NAME_PREFIX: &str = "MONYACODE_";
const MONYACODE_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::RunnableSymbol => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{MONYACODE_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::Custom(s) => write!(
                f,
                "{MONYACODE_VARIABLE_NAME_PREFIX}{MONYACODE_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of MonyaCode at the time the task was spawned.
...
/// Keeps all MonyaCode-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into MonyaCode.
...
type MonyaCodeEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, MonyaCodeEnvVariable>,
    commands: HashMap<VsCodeCommand, MonyaCodeEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, MonyaCodeEnvVariable>) -> Self {
        Self {
            variables,
    ...
    fn with_commands(mut self, commands: impl IntoIterator<Item = (VsCodeCommand, MonyaCodeEnvVariable)>) -> Self {
    ...
    // Replaces occurrences of VsCode-specific environment variables with MonyaCode equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->MonyaCode hit, perform a substitution
