//! The MonyaCode Rust Extension API allows you write extensions in Rust.
...
/// A result returned from a MonyaCode extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a MonyaCode task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, MonyaCode will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent MonyaCode from running second phase of resolution in such case.
...
/// Registers the provided type as a MonyaCode extension.
///
...
#[unsafe(link_section = "monyacode:api-version")]
#[doc(hidden)]
pub static MONYACODE_API_VERSION: [u8; 6] = *include_bytes!(concat!(env!("OUT_DIR"), "/version_bytes"));
