...
async fn load_keybind_context_language(workspace: WeakEntity<Workspace>, cx: &mut AsyncApp) -> Arc<Language> {
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("MonyaCode Keybind Context")
        })
        .context("Failed to load MonyaCode Keybind Context language")
        .log_err();
    ...
    language.unwrap_or_else(|| {
        Arc::new(Language::new(
            LanguageConfig {
                name: "MonyaCode Keybind Context".into(),
                ..Default::default()
            },
