...
use task::{DebugScenario, MonyaCodeDebugConfig, SpawnInTerminal, TaskTemplate};
...
pub struct WasmExtension {
    ...
    #[allow(unused)]
    pub monyacode_api_version: SemanticVersion,
...
impl extension::Extension for WasmExtension {
    ...
    async fn dap_config_to_scenario(&self, config: MonyaCodeDebugConfig) -> Result<DebugScenario> {
        self.call(|extension, store| {
            async move {
                let kind = extension
    ...
    pub fn load_extension(
        ...
        let compile_task = {
            ...
            executor.spawn(async move {
                let monyacode_api_version = parse_wasm_extension_version(&manifest_id, &wasm_bytes)?;
                let component =
                    Component::from_binary(&engine, &wasm_bytes).context("failed to compile wasm component")?;

                anyhow::Ok((monyacode_api_version, component))
            })
        };

        let load_extension = |monyacode_api_version: SemanticVersion, component| async move {
            ...
            anyhow::Ok((
                ...
                tx,
                monyacode_api_version,
            ))
        };

        cx.spawn(async move |cx| {
            let (monyacode_api_version, component) = compile_task.await?;

            // Run wasi-dependent operations on tokio.
            // wasmtime_wasi internally uses tokio for I/O operations.
            let (extension_task, manifest, work_dir, tx, monyacode_api_version) =
                gpui_tokio::Tokio::spawn(cx, load_extension(monyacode_api_version, component))?.await??;
...
pub fn parse_wasm_extension_version(extension_id: &str, wasm_bytes: &[u8]) -> Result<SemanticVersion> {
    ...
    for part in wasmparser::Parser::new(0).parse_all(wasm_bytes) {
        if let wasmparser::Payload::CustomSection(s) = part.context("error parsing wasm extension")?
            && (s.name() == "monyacode:api-version" || s.name() == "zed:api-version")
        {
            version = parse_wasm_extension_version_custom_section(s.data());
            if version.is_none() {
                bail!(
                    "extension {} has invalid monyacode:api-version or zed:api-version section: {:?}",
                    extension_id,
                    s.data()
                );
            }
    ...
    version.with_context(|| format!("extension {extension_id} has no monyacode:api-version or zed:api-version section"))
}
