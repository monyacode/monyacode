...
impl PythonDebugAdapter {
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use built-in MonyaCode terminal if user did not explicitly provide a setting for console.
            if console.is_null() {
        ...
        let result = self.base_venv_path
            .get_or_init(|| async {
                ...
                let output = util::command::new_smol_command(&base_python)
                    .args(["-m", "venv", "monyacode_base_venv"])
                    .current_dir(
                        &debug_adapter_path,
                    )
                    .spawn()
...
impl DebugAdapter for PythonDebugAdapter {
    ...
    async fn config_from_monyacode_format(&self, monyacode_scenario: MonyaCodeDebugConfig) -> Result<DebugScenario> {
        let mut args = json!({
            "request": match monyacode_scenario.request {
        ...
        match &monyacode_scenario.request {
            ...
            DebugRequest::Launch(launch) => {
                ...
                if let Some(stop_on_entry) = monyacode_scenario.stop_on_entry {
        ...
        Ok(DebugScenario {
            adapter: monyacode_scenario.adapter,
            label: monyacode_scenario.label,
            config: args,
            build: None,
            tcp_connection: None,
        })
            ...
            "properties": {
                ...
                "pathMappings": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "localRoot": {
                                "default": "${MONYACODE_WORKTREE_ROOT}",
                ...
                },
                {
                    ...
                    "then": {
                        "properties": {
                            ...
                            "envFile": {
                                "default": "${MONYACODE_WORKTREE_ROOT}/.env",
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because MonyaCode defaults to the cwd worktree root)
