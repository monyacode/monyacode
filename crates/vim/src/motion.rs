...
impl Vim {
    pub(crate) fn search_motion(&mut self, m: Motion, window: &mut Window, cx: &mut Context<Self>) {
        if let Motion::MonyaCodeSearchResult { prior_selections, .. } = &m {
            match self.mode {
...
impl Motion {
    fn default_kind(&self) -> MotionKind {
        ...
        match self {
            ...
            | SneakBackward { .. }
            | Jump { .. }
            | MonyaCodeSearchResult { .. } => MotionKind::Exclusive,
    ...
    pub fn move_point(
        ...
        let (new_point, goal) = match self {
            ...
            MonyaCodeSearchResult { new_selections, .. } => {
    ...
    pub fn range(
        ...
        if let Motion::MonyaCodeSearchResult {
            prior_selections,
            new_selections,
        } = self
        {
...
mod test {
    ...
    async fn test_matching(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_shared_state(indoc! {r"func ˇ(a string) {
            ...
            .await;
        cx.simulate_shared_keystrokes("%").await;
        cx.shared_state().await.assert_eq(indoc! {r"func (a stringˇ) {
                do(something(with<Types>.and_arrays[0, 2]))
            }"});
        ...
        cx.set_shared_state(indoc! {r"func (a string) ˇ{
            ...
            .await;
        cx.simulate_shared_keystrokes("%").await;
        cx.shared_state().await.assert_eq(indoc! {r"func (a string) {
            do(something(with<Types>.and_arrays[0, 2]))
            ˇ}"});
    ...
    async fn test_matching_quotes_disabled(cx: &mut gpui::TestAppContext) {
        ...
        cx.update(|_, cx| {
            cx.bind_keys([KeyBinding::new("%", Matching { match_quotes: false }, None)]);
        });
    ...
    async fn test_unmatched_forward(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_shared_state(indoc! {r"func (a string) { a((b, cˇ))}"}).await;
        cx.simulate_shared_keystrokes("] )").await;
        cx.shared_state()
            .await
    ...
    async fn test_unmatched_backward(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_shared_state(indoc! {r"func (a string) {
            ...
            .await;
        cx.simulate_shared_keystrokes("[ {").await;
        cx.shared_state().await.assert_eq(indoc! {r"func (a string) ˇ{
                do(something(with<Types>.and_arrays[0, 2]))
            }"});
    ...
    async fn test_matching_tags(cx: &mut gpui::TestAppContext) {
        ...
        cx.simulate_shared_keystrokes("%").await;
        cx.shared_state().await.assert_eq(indoc! {r"<body><ˇ/body>"});
        cx.simulate_shared_keystrokes("%").await;

        // test jumping backwards
        cx.shared_state().await.assert_eq(indoc! {r"<ˇbody></body>"});
        ...
        cx.set_shared_state(indoc! {r"<div class='test' ˇid='main'>
            ...
            .await;
        cx.simulate_shared_keystrokes("%").await;
        cx.shared_state().await.assert_eq(indoc! {r"<div class='test' id='main'>
            <ˇ/div>
            "});
