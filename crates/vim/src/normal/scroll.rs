...
pub fn register(editor: &mut Editor, cx: &mut Context<Vim>) {
    ...
    Vim::action(editor, cx, |vim, _: &HalfPageRight, window, cx| {
        vim.scroll(false, window, cx, |c| ScrollAmount::PageWidth(c.unwrap_or(0.5)))
    });
...
fn scroll_editor(
    ...
    editor.change_selections(SelectionEffects::no_scroll().nav_history(false), window, cx, |s| {
        s.move_with(|map, selection| {
            ...
            let vertical_scroll_margin = (vertical_scroll_margin as u32).min(visible_line_count as u32 / 2);
            ...
            let max_visible_row = top
                .row()
                .0
                .saturating_add((visible_line_count as u32).saturating_sub(1 + vertical_scroll_margin));
            // scroll off the end.
            let max_row = if top.row().0 + visible_line_count as u32 >= max_point.row().0 {
                max_point.row()
            } else {
                DisplayRow((top.row().0 + visible_line_count as u32).saturating_sub(1 + vertical_scroll_margin))
            };
...
mod test {
    ...
    async fn test_scroll(cx: &mut gpui::TestAppContext) {
        ...
        let margin = cx
            ...
            .unwrap();
        cx.simulate_window_resize(cx.window, size(px(1000.), margin + 8. * line_height - px(1.0)));
        ...
        cx.update_editor(|editor, window, cx| assert_eq!(editor.snapshot(window, cx).scroll_position(), point(0., 0.)));
        cx.simulate_keystrokes("ctrl-e");
        cx.update_editor(|editor, window, cx| assert_eq!(editor.snapshot(window, cx).scroll_position(), point(0., 1.)));
        cx.simulate_keystrokes("2 ctrl-e");
        cx.update_editor(|editor, window, cx| assert_eq!(editor.snapshot(window, cx).scroll_position(), point(0., 3.)));
        cx.simulate_keystrokes("ctrl-y");
        cx.update_editor(|editor, window, cx| assert_eq!(editor.snapshot(window, cx).scroll_position(), point(0., 2.)));
        ...
        cx.update_editor(|editor, window, cx| {
            assert_eq!(editor.snapshot(window, cx).scroll_position(), point(0., 3.0));
            assert_eq!(
                editor.selections.newest(&editor.display_snapshot(cx)).range(),
    ...
    async fn test_ctrl_d_u(cx: &mut gpui::TestAppContext) {
        ...
        cx.shared_state().await.assert_matches();
        cx.simulate_shared_keystrokes("ctrl-d ctrl-d 4 j ctrl-u ctrl-u").await;
        cx.shared_state().await.assert_matches();

        // test returning to top
        cx.simulate_shared_keystrokes("g g ctrl-d ctrl-u ctrl-u").await;
        cx.shared_state().await.assert_matches();
    }
