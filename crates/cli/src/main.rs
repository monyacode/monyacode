...
const URL_PREFIX: [&'static str; 5] = ["monyacode://", "http://", "https://", "file://", "ssh://"];

struct Detect;

trait InstalledApp {
    fn monyacode_version_string(&self) -> String;
...
#[command(
    ...
    before_help = "The MonyaCode CLI binary.
This CLI is a separate binary that invokes MonyaCode.

Examples:
    `monyacode`
          Simply opens MonyaCode
    `monyacode --foreground`
          Runs in foreground (shows all logs)
    `monyacode path-to-your-project`
          Open your project in MonyaCode
    `monyacode -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | monyacode -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/MonyaCode`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\MonyaCode`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/monyacode`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in MonyaCode (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print MonyaCode's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run monyacode in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to MonyaCode.app or the editor binary
    #[arg(long)]
    monyacode: Option<PathBuf>,
    ...
    wsl: Option<String>,
    /// Not supported in MonyaCode CLI, only supported on MonyaCode binary
    ...
    diff: Vec<String>,
    /// Uninstall MonyaCode from user system
...
///
/// This method must return an absolute path, as many monyacode
...
fn main() -> Result<()> {
    ...
    let app = Detect::detect(args.monyacode.as_deref()).context("Bundle detection")?;

    if args.version {
        println!("{}", app.monyacode_version_string());
        return Ok(());
    }
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the MonyaCode CLI, only on MonyaCode binary.",
    ...
    if args.uninstall {
        ...
        let status = std::process::Command::new("sh")
            .arg(&script_path)
            .env("MONYACODE_CHANNEL", &*release_channel::RELEASE_CHANNEL_NAME)
            .status()
    ...
    let (server, server_name) = IpcOneShotServer::<IpcHandshake>::new().context("Handshake before MonyaCode spawn")?;
    let url = format!("monyacode-cli://{server_name}");
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            ...
            // On Linux, the desktop entry uses `cli` to spawn `monyacode`.
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                let (_, handshake) = server.accept().context("Handshake after MonyaCode spawn")?;
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/monyacode is for Arch (and other non-libexec distros),
                // ./monyacode is for the target directory in development builds.
                let possible_locations = [
                    "../libexec/monyacode-editor",
                    "../lib/monyacode/monyacode-editor",
                    "./monyacode",
                ];
                possible_locations
                    .iter()
    ...
    impl InstalledApp for App {
        fn monyacode_version_string(&self) -> String {
            format!(
                ...
                match option_env!("MONYACODE_COMMIT_NAME") {
                ...
                match option_env!("MONYACODE_COMMIT_SHA") {
        ...
        fn launch(&self, ipc_url: String, user_data_dir: Option<&str>) -> anyhow::Result<()> {
            ...
            let sock_path = data_dir.join(format!("monyacode-{}.sock", *release_channel::RELEASE_CHANNEL_NAME));
...
mod flatpak {
    ...
    const EXTRA_LIB_ENV_NAME: &str = "MONYACODE_FLATPAK_LIB_PATH";
    const NO_ESCAPE_ENV_NAME: &str = "MONYACODE_FLATPAK_NO_ESCAPE";
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=MONYACODE_NO_BUNDLED_UNINSTALL=Please use flatpak to uninstall monyacode".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("monyacode").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--monyacode";
            }

            if !is_app_location_set {
                args.push("--monyacode".into());
                args.push(flatpak_dir.join("libexec").join("monyacode-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("app.liten.MonyaCode"))
            && args.monyacode.is_none()
        {
            args.monyacode = Some("/app/libexec/monyacode-editor".into());
            unsafe { env::set_var("MONYACODE_NO_BUNDLED_UNINSTALL", "Use flatpak to uninstall monyacode") };
        }
        args
    }
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("app.liten.MonyaCode") {
                return None;
            }
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // ../MonyaCode.exe is the standard, lib/monyacode is for MSYS2, ./monyacode.exe is for the target
                // directory in development builds.
                let possible_locations = [
                    "../MonyaCode.exe",
                    "../lib/monyacode/monyacode-editor.exe",
                    "./monyacode.exe",
                ];
                possible_locations
                    .iter()
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn monyacode_version_string(&self) -> String {
            format!("MonyaCode {} – {}", self.version(), self.path().display(),)
        }
        ...
        fn launch(&self, url: String, user_data_dir: Option<&str>) -> anyhow::Result<()> {
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    // equivalent to: open monyacode-cli:... -a /Applications/MonyaCode\ Preview.app
                ...
                Self::LocalPath { executable, .. } => {
                    ...
                    let subprocess_stdout_file = fs::File::create(executable_parent.join("monyacode_dev.log"))
        ...
        fn run_foreground(&self, ipc_url: String, user_data_dir: Option<&str>) -> io::Result<ExitStatus> {
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/monyacode"),
    ...
    impl Bundle {
        fn version(&self) -> String {
            match self {
                ...
                Self::LocalPath { .. } => match option_env!("MONYACODE_COMMIT_NAME") {
                    Some(commit_name) => format!("{commit_name} "),
                    None => match option_env!("RELEASE_VERSION") {
                        Some(version) => format!("{version} "),
