...
use assets::Assets;
use monyacode::{
    OpenListener, OpenRequest, RawOpenRequest, app_menus, build_window_options, derive_paths_with_position,
    handle_cli_connection, handle_keymap_file_changes, handle_settings_file_changes, initialize_workspace,
...
use uuid::Uuid;
use workspace::{
    AppState, PathList, SerializedWorkspaceLocation, Toast, Workspace, WorkspaceSettings, WorkspaceStore,
    notifications::NotificationId,
};

use crate::monyacode::{OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "MonyaCode failed to launch";
    ...
    Application::new().with_quit_mode(QuitMode::Explicit).run(move |cx| {
        if let Ok(window) = cx.open_window(gpui::WindowOptions::default(), |_, cx| cx.new(|_| gpui::Empty)) {
            window
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!("MonyaCode failed to open a window: {e:?}. See monyacode://docs/linux for troubleshooting steps.");
        ...
        any(target_os = "linux", target_os = "freebsd") => {
            ...
            _cx.spawn(async move |_cx| {
                ...
                proxy
                    .add_notification(
                        notification_id,
                        Notification::new("MonyaCode failed to launch")
                            .body(Some(
                                format!("{e:?}. See monyacode://docs/linux for troubleshooting steps.").as_str(),
                            ))
...
pub fn main() {
    ...
    // `monyacode --askpass` Makes monyacode operate in nc/netcat mode for use with askpass
    ...
    // `monyacode --crash-handler` Makes monyacode operate in minidump crash handler mode
    if let Some(socket) = &args.crash_handler {
        crashes::crash_server(socket.as_path());
        return;
    }

    // `monyacode --nc` Makes monyacode operate in nc/netcat mode
    ...
    // `monyacode --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    match util::get_monyacode_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("MONYACODE_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    let version_name = match option_env!("MONYACODE_COMMIT_NAME") {
    ...
    let app_commit_sha =
        option_env!("MONYACODE_COMMIT_SHA").map(|commit_sha| AppCommitSha::new(commit_sha.to_string()));

    if args.system_specs {
        let system_specs =
            system_specs::SystemSpecs::new_stateless(app_version, app_commit_sha, *release_channel::RELEASE_CHANNEL);
        println!("MonyaCode System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    app.background_executor()
        .spawn(crashes::init(InitCrashHandler {
            session_id,
            monyacode_version: app_version.to_string(),
            binary: "monyacode".to_string(),
    ...
    let failed_single_instance_check =
        if std::env::var("MONYACODE_STATELESS").is_ok() || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev {
            false
        } else {
            cfg_select! {
                any(target_os = "linux", target_os = "freebsd") => {
                    crate::monyacode::listen_for_cli_connections(open_listener.clone()).is_err()
                }
                windows => {
                    !crate::monyacode::windows_only_instance::handle_single_instance(open_listener.clone(), &args)
                }
                target_os = "macos" => {
                    use monyacode::mac_only_instance::*;
                    ensure_only_instance() != IsOnlyInstance::Yes
                }
            }
        };
    if failed_single_instance_check {
        println!("monyacode is already running");
        return;
    }
    ...
    let user_settings_file_rx =
        watch_config_file(&app.background_executor(), fs.clone(), paths::settings_file().clone());
    ...
    app.run(move |cx| {
        ...
        let http = {
            ...
            ReqwestClient::proxy_and_user_agent(proxy_url, &user_agent).expect("could not start HTTP client")
        };
                ...
                Arc::new(move |cx: &mut App| {
                    workspace_store.update(cx, |workspace_store, cx| {
                        workspace_store
                            ...
                            .iter()
                            .map(|workspace| {
                                workspace.update(cx, |workspace, _, cx| workspace.project().read(cx).lsp_store())
                            })
                            .collect()
                    })
                })
        ...
        cx.observe_global::<SettingsStore>({
            move |cx| {
                ...
                cx.set_text_rendering_mode(match WorkspaceSettings::get_global(cx).text_rendering_mode {
        ...
        let urls: Vec<_> = args.paths_or_urls.iter().map(|arg| parse_url_arg(arg, cx)).collect();
        ...
        crate::monyacode::component_preview::init(app_state.clone(), cx);
...
fn handle_open_request(request: OpenRequest, app_state: Arc<AppState>, cx: &mut App) {
    if let Some(kind) = request.kind {
        match kind {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| {
                        window.dispatch_action(
                            Box::new(app_actions::Extensions {
                    ...
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let lsp_store = workspace.update(cx, |workspace, cx| {
                                workspace.project().update(cx, |project, _| project.lsp_store())
                            })?;
                            let json_schema_content = json_schema_store::resolve_schema_request_inner(
                                &app_state.languages,
                                lsp_store,
                                &schema_path,
                                cx,
            ...
            OpenRequestKind::Setting { setting_path } => {
                // monyacode://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // monyacode://settings/languages/Rust/tab_size  - SUPPORT
    ...
    if !request.open_paths.is_empty() || !request.diff_paths.is_empty() {
        ...
        task = Some(cx.spawn(async move |cx| {
            let paths_with_position = derive_paths_with_position(app_state.fs.as_ref(), request.open_paths).await;
...
pub(crate) async fn restorable_workspace_locations(
    ...
    if last_session_id.is_none() && matches!(restore_behavior, workspace::RestoreOnStartupBehavior::LastSession) {
        restore_behavior = workspace::RestoreOnStartupBehavior::LastWorkspace;
    }

    match restore_behavior {
        workspace::RestoreOnStartupBehavior::LastWorkspace => workspace::last_opened_workspace_location()
            .await
...
#[derive(Parser, Debug)]
#[command(name = "monyacode", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `monyacode://` scheme, or relative to <https://codeberg.org>.
    ...
    /// On macOS, the default is `~/Library/Application Support/MonyaCode`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/monyacode`.
    /// On Windows, the default is `%LOCALAPPDATA%\MonyaCode`.
    #[arg(long, value_name = "DIR", verbatim_doc_comment)]
    user_data_dir: Option<String>,

    /// The username and WSL distribution to use when opening paths. If not specified,
    /// MonyaCode will attempt to open the paths directly.
    ///
    ...
    /// prevents MonyaCode from starting, so you can't run `monyacode: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used to remove the need for netcat as a dependency by having MonyaCode act like netcat
    /// communicating over a Unix socket.
    #[arg(long, hide = true)]
    nc: Option<String>,

    /// Used for recording minidumps on crashes by having MonyaCode run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run monyacode in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// by having MonyaCode act like netcat communicating over a Unix socket.
    #[arg(long)]
    #[cfg(not(target_os = "windows"))]
    #[arg(hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("monyacode://")
                || arg.starts_with("monyacode-cli://")
                || arg.starts_with("ssh://")
            {
                arg.into()
            } else {
...
fn load_embedded_fonts(cx: &App) {
    ...
    cx.text_system().add_fonts(embedded_fonts.into_inner()).unwrap();
}
...
fn check_for_conpty_dll() {
    ...
    if let Ok(hmodule) = unsafe { LoadLibraryW(w!("conpty.dll")) } {
        unsafe {
            FreeLibrary(hmodule).context("Failed to free conpty.dll").log_err();
        }
    } else {
