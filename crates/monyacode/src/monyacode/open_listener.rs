...
impl OpenRequest {
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("monyacode-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("monyacode-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("monyacode://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("monyacode://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("monyacode://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if let Some(schema_path) = url.strip_prefix("monyacode://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if let Some(action) = url.strip_prefix("monyacode://kb/") {
                this.kind = Some(OpenRequestKind::Keybinding {
                    action: action.to_string(),
                });
            } else if let Some(action) = url.strip_prefix("monyacode://action/") {
                this.kind = Some(OpenRequestKind::Keybinding {
                    action: action.to_string(),
                });
            } else if url == "monyacode://settings" || url == "monyacode://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("monyacode://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(path) = url.strip_prefix("monyacode://docs/") {
                let path = if path == "" {
                    "index.md".to_string()
                } else {
                    path.to_string()
            ...
            } else if let Some(clone_path) = url.strip_prefix("monyacode://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("monyacode://git/commit/") {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("monyacode-{}.sock", *RELEASE_CHANNEL_NAME));
...
// TODO: This and crates/monyacode/src/main.rs:restore_or_create_workspace() are almost
...
mod tests {
    use super::*;
    use crate::monyacode::{open_listener::open_local_workspace, tests::init_test};
    use cli::{
        CliResponse,
        ipc::{self},
    };
    ...
    async fn test_open_workspace_with_directory(cx: &mut TestAppContext) {
        ...
        // Now open a file inside that workspace, but tell MonyaCode to open a new window
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell MonyaCode to add it to the current workspace
        ...
        // but tell MonyaCode to NOT add it to the current workspace
    ...
    fn test_parse_git_clone_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["monyacode://git/clone/?repo=https://github.com/monyacode/monyacode.git".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        match request.kind {
            Some(OpenRequestKind::GitClone { repo_url }) => {
                assert_eq!(repo_url, "https://github.com/monyacode/monyacode.git");
            }
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["monyacode://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fmonyacode%2Fmonyacode.git".into()],
                    ..Default::default()
                },
                cx,
            )
