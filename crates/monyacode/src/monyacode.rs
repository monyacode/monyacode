...
use app_actions::{About, OpenBrowser, OpenMonyaCodeUrl, OpenServerSettings, OpenSettingsFile, Quit, ToggleTitleBar};
pub use app_menus::*;
use assets::Assets;
...
actions!(
    ...
    [
        ...
        OpenDefaultSettings,
        /// Opens the MonyaCode repository on GitHub.
        OpenMonyaCodeRepo,
...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &OpenMonyaCodeRepo, cx| cx.open_url("https://github.com/monyacode/monyacode"));
    cx.on_action(quit);
...
fn get_window_decorations(cx: &mut App) -> Option<gpui::WindowDecorations> {
    Some(match std::env::var("MONYACODE_WINDOW_DECORATIONS") {
            ...
            any(target_os = "linux", target_os = "freebsd") => {
                let message = format!(
                    db::indoc! {r#"
                            ...
                            This may be due to system-wide limits on inotify instances. For troubleshooting see: monyacode://docs/linux
                            "#},
                    e
                );
                ...
                cx.spawn(async move |_, cx| {
                    if prompt.await == Ok(0) {
                        cx.update(|cx| {
                            cx.open_url("monyacode://docs/linux#could-not-start-inotify");
                            cx.quit();
                        })
                        .ok();
                    }
            ...
            target_os = "windows" => {
                let message = format!(
                    db::indoc! {r#"
                            ...
                            This may occur on network filesystems and WSL paths. For troubleshooting see: monyacode://docs/windows
                            "#},
                    e
                );
                let prompt = window.prompt(
                ...
                cx.spawn(async move |_, cx| {
                    if prompt.await == Ok(0) {
                        cx.update(|cx| {
                            cx.open_url("monyacode://docs/windows");
                            cx.quit()
                        })
                        .ok();
                    }
...
fn show_software_emulation_warning_if_needed(specs: gpui::GpuSpecs, window: &mut Window, cx: &mut Context<Workspace>) {
    if specs.is_software_emulated && std::env::var("MONYACODE_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            ("DirectX", "monyacode://docs/windows", "monyacode://docs/windows")
        } else {
            ("Vulkan", "monyacode://docs/linux", "monyacode://docs/linux")
        };
        let message = format!(
            db::indoc! {r#"
            MonyaCode uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set MONYACODE_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
...
fn register_actions(app_state: Arc<AppState>, workspace: &mut Workspace, _: &mut Window, cx: &mut Context<Workspace>) {
    workspace
        ...
        .register_action(|_, action: &OpenMonyaCodeUrl, _, cx| {
        ...
        .register_action(|workspace, action: &app_actions::OpenRemote, window, cx| {
        ...
        .register_action(|_, _: &install_cli::RegisterUriScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_uri_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterUriScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterUriScheme>(),
                            format!(
                                "monyacode:// links will now open in {}.",
            ...
            .detach_and_prompt_err("Error registering monyacode:// scheme", window, cx, |_, _, _| None);
        })
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &app_actions::project_panel::ToggleFocus,
             window: &mut Window,
...
fn open_about(cx: &mut App) {
    const MONYACODE_UPDATE_EXPLANATION: &str = match option_env!("MONYACODE_UPDATE_EXPLANATION") {
        Some(text) => text,
    ...
    impl AboutWindow {
        fn new(cx: &mut Context<Self>) -> Self {
            let version = match option_env!("MONYACODE_COMMIT_NAME") {
                Some(commit_name) => commit_name,
                None => env!("CARGO_PKG_VERSION"),
            };

            let debug = if cfg!(debug_assertions) { "(debug)" } else { "" };
            let headline: SharedString = format!("MonyaCode v{version} {debug}").into();
                ...
                v_flex()
                    ...
                    .child(
                        v_flex()
                            ...
                            .child(
                                Label::new(MONYACODE_UPDATE_EXPLANATION)
                                    .italic()
                                    .size(LabelSize::XSmall),
                            )
                            .child(
                                ButtonLink::new("monyacode-editor.com", "https://monyacode-editor.com")
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &app_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .monyacode/settings.jsonc containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.monyacode/settings.jsonc"))
            .await
            .unwrap();
        ...
        // 3. Add .monyacode to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions = Some(vec![".monyacode".to_string()]);
            });
        });

        eprintln!("Added .monyacode to file_scan_exclusions in settings");

        // 4. Run tasks to apply settings
        cx.background_executor.run_until_parked();

        // 5. Critical: Verify .monyacode is actually excluded from worktree
        let worktree = cx.update(|cx| project.read(cx).worktrees(cx).next().unwrap());

        let has_entry = cx.update(|cx| worktree.read(cx).entry_for_path(rel_path(".monyacode")).is_some());

        eprintln!(
            "Is .monyacode directory visible in worktree after exclusion: {}",
            has_entry
        );

        // This assertion verifies the test is set up correctly to show the bug
        // If .monyacode is not excluded, the test will fail here
        assert!(
            !has_entry,
            "Test precondition failed: .monyacode directory should be excluded but was found in worktree"
        );
