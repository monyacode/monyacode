...
impl WgpuContext {
    pub fn new() -> anyhow::Result<Self> {
        let device_id_filter = match std::env::var("MONYACODE_DEVICE_ID") {
            Ok(val) => parse_pci_id(&val)
                .context("Failed to parse device ID from `MONYACODE_DEVICE_ID` environment variable")
                .log_err(),
            Err(std::env::VarError::NotPresent) => None,
            err => {
                err.context("Failed to read value of `MONYACODE_DEVICE_ID` environment variable")
                    .log_err();
                None
            }
        };
        ...
        if let Some(device_id) = device_id_filter {
            ...
            for adapter in adapters.into_iter() {
                ...
                if info.device == device_id {
                    log::info!("Found GPU matching MONYACODE_DEVICE_ID={:#06x}: {}", device_id, info.name);
                    return Ok(adapter);
                } else {
            ...
            log::warn!(
                "No GPU found matching MONYACODE_DEVICE_ID={:#06x}. Available devices:",
                device_id
            );
