...
/// On Windows, this is set to the CLI variant of monyacode
static ASKPASS_PROMONYACODE: OnceLock<std::path::PathBuf> = OnceLock::new();
...
impl PasswordProxy {
    pub async fn new(
        ...
        let temp_dir = tempfile::Builder::new().prefix("monyacode-askpass").tempdir()?;
        ...
        let askpass_program = ASKPASS_PROMONYACODE.get_or_init(|| current_exec);
...
/// The main function for when MonyaCode is running in netcat mode for use in askpass.
...
pub fn set_askpass_program(path: std::path::PathBuf) {
    if ASKPASS_PROMONYACODE.set(path).is_err() {
