...
pub fn init(app_state: Arc<AppState>, cx: &mut App) {
    cx.on_action(|_: &app_actions::dev::ToggleInspector, cx| {
...
fn render_inspector_id(inspector_id: &InspectorElementId, cx: &App) -> Div {
    ...
    let source_location_string = source_location_string
        .strip_prefix(env!("MONYACODE_REPO_DIR"))
    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running MonyaCode CLI"))
...
async fn open_editor_source_location(location: &'static std::panic::Location<'static>) -> anyhow::Result<()> {
    let mut path = Path::new(env!("MONYACODE_REPO_DIR")).to_path_buf();
    path.push(Path::new(location.file()));
    let path_arg = format!("{}:{}:{}", path.display(), location.line(), location.column());

    let output = new_smol_command("monyacode")
        .arg(&path_arg)
        .output()
        .await
        .with_context(|| format!("running monyacode to open {path_arg} failed"))?;

    if !output.status.success() {
        Err(anyhow!(
            "running monyacode to open {path_arg} failed with stderr: {}",
