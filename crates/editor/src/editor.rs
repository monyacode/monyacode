...
//! The main point of interest in this crate is [`Editor`] type, which is used in every other MonyaCode part as a user input element.
...
/// MonyaCode's primary implementation of text input, allowing users to edit a [`MultiBuffer`].
///
...
impl Editor {
        ...
        Self::new_in_workspace(workspace, window, cx).detach_and_prompt_err(
            ...
            |e, _, _| match e.error_code() {
                ErrorCode::RemoteUpgradeRequired => Some(format!(
                    "The remote instance of MonyaCode does not support this yet. It must be upgraded to {}",
    ...
    fn trigger_on_type_formatting(
        ...
        // OnTypeFormatting returns a list of edits, no need to pass them between MonyaCode instances,
    ...
    pub fn diff_clipboard_with_selection(
        ...
        if selections.is_empty() {
            log::warn!("There should always be at least one selection in MonyaCode. This is a bug.");
            return;
        };
    ...
    fn initialize_new_minimap(
        ...
        const MINIMAP_FONT_FAMILY: SharedString = SharedString::new_static(".MonyaCodeMono");
